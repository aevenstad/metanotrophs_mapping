---
title: "Geographic Coordinates Cleanup"
author: "Your Name"
date: "`r Sys.Date()`"
...
setwd("/Users/aev021/OneDrive - UiT Office 365/UiT/projects/methanotrophs_project")
```


# Load Tilman filtered accessions
```{r}
tilman_data <- read.table(
  "ncbi/ncbi_search_10_06/entries_with_envir_category.txt",
  sep = "\t",
  header = TRUE,
...
bad_coords <- tilman_data[!str_detect(tilman_data$Latitude_and_longitude, pattern), ]
good_coords <- tilman_data[str_detect(tilman_data$Latitude_and_longitude, pattern), ]
```
...
```{r}
write.table(final_coords, file = "summary_table_clean_coordinates.txt", row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
```
